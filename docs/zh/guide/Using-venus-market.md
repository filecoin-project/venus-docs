...
- venus-gateway组件服务,对应命令行`signer-url`, `signer-token`和配置中的`[Signer]`项

#### 相关token配置

- `auth-token`项,是从venus-auth服务上获取,用于验证venus-market访问venus,venus-messager的访问权限.
  ...
  - 创建新token:
  ```shell
  ./venus-auth token gen token-01 --perm admin
  generate token success:eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoibWFya2V0LWNsaWVudCIsInBlcm0iOiJzaWduIiwiZXh0IjoiIn0.8lSYccRSO-nZuAoH--1s2HG11HUxSvRLDWdpvXkdV-4
  ```

- `signer-token`项,为venus-wallet注册到venus-gateway服务上,用于验证market通过gateway来访问wallet来进行签名时用于在wallet端验证访问权限的token.

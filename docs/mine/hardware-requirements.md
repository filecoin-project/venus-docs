...
Additional GPU models need to be enabled manually ([instructions for Venus](venus/gpus.md)).
...
Additional hard drives for the final storage of "sealed sectors", the Venus chain etc. will be needed as well.
...
| -------------------------- | ------------------------------ | -------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sealing: preCommit phase 1 | Yes (1 core or 1 core-complex) | No       | 128GiB                 | PoRep SDR encoding. Not amenable to parallelization. Core usage depends on value of [`FIL_PROOFS_USE_MULTICORE_SDR`](https://github.com/filecoin-project/rust-fil-proofs/). Also see the [Venus seal workers guide](venus/seal-workers.md). |
...
Note that the [Venus](venus/README.md) implementation allows to configure and delegate specific sealing phases to [Venus workers](venus/seal-workers.md).
